...
def score_item(section: str, title: str, summary: str, url: str, published: datetime, cfg: dict) -> float:
    ...
    score += 8 if any(domain.endswith(d) for d in cfg["preferred_domains"]) else 0
...
def collect(cfg: dict) -> list[Article]:
    ...
    collection_order = ["부동산"] + [name for name in cfg["sections"] if name != "부동산"]
    for section in collection_order:
        ...
        candidates.sort(key=lambda x: (x.score, x.published_at), reverse=True)
        selected = candidates[: cfg["max_articles_per_section"]]
